...
date: "12 Oct 2021"
author: "Reading Time: About 8 minutes, Authors: Karim El Asmar, Stepan Emelianenko, Nelly Gray, Ziqi Li, Ray Park, Shirley Wan, Ditlev Meulengracht"
output:
  html_document:
    highlight: zenburn
    theme: flatly
...
The 95% confindence interval for the mean payment of 2 people spending 4 nights is 376-387 $. 

For reference we wanna see the statistics of the actual values: 

```{r}
favstats(targets$price_4_nights)
```

In comparison, the actual mean is $401 which is close to our model, and the differences makes sense given there is only a 50% explanatory power in our model. 

We hypothesize that our model may be improved by finding a variable that can explain high value outliers, however, we have not come across such a relationship in any of the variables we looked into in the EDA. 
