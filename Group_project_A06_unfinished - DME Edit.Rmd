...
#Create the model1
model1 <- lm(log(price_4_nights) ~ prop_type_simplified +number_of_reviews +review_scores_rating ,data=listings_4)
summary(model1)
...
```{r}
#Create the model1
model2 <- lm(log(price_4_nights) ~ prop_type_simplified +number_of_reviews +review_scores_rating + room_type ,data=listings_4)
summary(model2)
```
...
```
WE surprisingly see that VIF is not too high between beds, bedrooms, and accomodates so we keep them in for now.
...
model3_2 <- lm(log(price_4_nights) ~ prop_type_simplified+number_of_reviews+review_scores_rating + room_type+bedrooms + beds + accommodates, data = listings_4)
...
```{r}
###
model3_3 <- lm(log(price_4_nights) ~ number_of_reviews+review_scores_rating + room_type+bedrooms + beds + accommodates, data = listings_4)
model3_4 <- lm(log(price_4_nights) ~ prop_type_simplified+number_of_reviews+review_scores_rating + bedrooms + beds + accommodates, data = listings_4)
...
```{r}
model4_1 <- lm(log(price_4_nights) ~ number_of_reviews+review_scores_rating + room_type+bedrooms + beds + accommodates + host_is_superhost, data = listings_4)
summary(model4_1)
...
```{r}
model5_1 <- lm(log(price_4_nights) ~ number_of_reviews+review_scores_rating + room_type+bedrooms + beds + accommodates + host_is_superhost + instant_bookable, data = listings_4)
summary(model5_1)
...
```{r}
model6_1 <- lm(log(price_4_nights) ~ number_of_reviews+review_scores_rating + room_type+bedrooms + beds + accommodates + host_is_superhost + instant_bookable + neighbourhood_cleansed, data = listings_4)
summary(model6_1)
```
...
```{r}
model7_1 <- lm(log(price_4_nights) ~ number_of_reviews+review_scores_rating + room_type+bedrooms + beds + accommodates + host_is_superhost + instant_bookable + neighbourhood_cleansed + availability_30, data = listings_4)
summary(model7_1)
```
...
```{r}
model8_1 <- lm(log(price_4_nights) ~ number_of_reviews+review_scores_rating + room_type+bedrooms + beds + accommodates + host_is_superhost + instant_bookable + neighbourhood_cleansed + availability_30 + license, data = listings_4)
summary(model8_1)
```
...
Given the above model beds is no longer significant so we would like to take that out. 

```{r}
model9_1 <- lm(log(price_4_nights) ~ number_of_reviews+review_scores_rating + room_type+bedrooms + accommodates + host_is_superhost + instant_bookable + neighbourhood_cleansed + availability_30 + license, data = listings_4)
summary(model9_1)
```
...
Variance inflation factor is below 5 for all variables and we keep them in for now. 
...
```{r}
huxreg(list("Prop Type, Reviews, Rating" = model1, "+ Room Type" = model2))
       
       
huxreg(list("+ Beds + Bedrooms + Accom." = model3_2, "- Prop Type" = model3_3, " - Room Type + Prop Type"  = model3_4))


```
From here we decided to go with the model without Prop Type 

```{r}

huxreg(list("+ Superhost" = model4_1, "+ immidiate bookings" = model5_1, "+ Neighbourhoods" = model6_1, "+ Availability" = model7_1, "+ License" = model8_1,"- Beds" = model9_1))
```

Given the above, it seems both beds and license can be removed without any material effect. 

```{r}
model_final <- lm(log(price_4_nights) ~ number_of_reviews+review_scores_rating + room_type+bedrooms + accommodates + host_is_superhost + instant_bookable + neighbourhood_cleansed + availability_30, data = listings_4)
summary(model_final)
```
...
```{r}
huxreg(list("All Variables"=model8_1, "- License"=model7_1,  "- Beds" = model9_1, "-Beds - License (final)" = model_final))
```



### Training in Testing sets.
