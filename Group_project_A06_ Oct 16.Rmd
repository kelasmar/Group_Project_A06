...
Interestingly, superhosts seem to have a higher price than non-superhosts. Let's plot some relationships to explore this further.
...
Interestingly we see a very strong correlation between both of these to ratings. So far so good so our past conclusions still hold true. 
...
ggplot(listings2,
  ...
  NULL
```
It seems there are more variation amongst non-superhosts. Let's confirm this:
...
The two distributions look similar but it is worth noting some skew in superhosts. 
...
WE see that as the number of reviews increase the price falls, likely because these are less premium properties. This will be useful for our regression models. 
...
```
We see that licenses have higher mean and median prices. 
...
First let's visualize the means
...
This is super useful but let's also take a look at the distribution: 
...
This seems to hold true across a variety of categories and we will not considerit going forward. 


### Last Review


Let's also look at whether last review date hast an impact
...
Here we see a slight relationship and newer properties with newer reviews are higher priced. 

We also not that there has been some properties that have not been reviewed since what looks like the covid outbreak. W

We will plt the density for reference
...
It seems that some properties have not had reviews since Covid. This is an interesting perspective but we will still be including them in our sample. 
...
This model has an adjusted r square of 37.6% and adds variables beds, bedrooms and accommodates. 
...
> Below is old code that could like be moved up in EDA part. 


```{r}
##Is the price of private room significantly lower?
listings_private<-listings_4 %>% 
  mutate(is_private_room = ifelse(prop_type_simplified=="Private room in rental unit", "yes","no")) #creating categories
